...
* **HonomiTV Web UI からのリモート操作に対応**: ペアリングした HonomiTV の Web UI から、ライブ放送・録画番組を開く操作に加えて、再生・一時停止・停止・音量・シークを操作できます。録画再生中（追いかけ再生を含む）は再生位置・長さ・CM 区間を Web UI へ通知し、進捗バーのドラッグによる任意位置へのシーク、チャプター送り／戻し、CM 区間から本編への復帰、CM スキップ設定（オフ／手動／自動）の切り替えを受け付けます。
