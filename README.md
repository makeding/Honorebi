...
## このフォークについて
このリポジトリは、Komorebi を Android TV / Fire TV で日常利用しやすくするために調整した fork です。上流の 1.1.0-beta 系の変更を取り込みつつ、既存のプレイヤー操作感を残し、視聴中のレスポンス、リモコン操作、字幕・録画再生まわりを重点的に直しています。

主な変更点は以下です。

* アプリ名と applicationId を Honorebi として分離し、元の Komorebi と共存しやすくしました。
* ライブ視聴のサブメニュー、字幕同期、ロゴ読み込みなどを見直し、視聴中の UI 負荷を削減しました。
* リモコンで使いやすいクイックチャンネル切替を追加し、GR と放送中アニメを縦方向に分けて移動量を減らしました。
* 追いかけ再生をライブ画面から直接開けるようにし、既存の録画プレイヤー経路へ自然に接続しました。
* ARIB 字幕の native デコードとオーバーレイ表示を追加し、字幕の表示・自動消去の挙動を調整しました。
* SMB 再生から libVLC 依存を外し、既存の ExoPlayer ベースの動画再生経路へ統合しました。
* `AppContentStore` でチャンネル・録画情報を共有し、プレイヤー表示中の重い再取得や画面遷移時の待ちを減らしました。
* AI コンシェルジュなど、自分の利用に不要な重い機能は無効化しています。
